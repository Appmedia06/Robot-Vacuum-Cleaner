...
//	MPU6050_WriteReg(MPU6050_PWR_MGMT_1, 0x01); // 電源管理寄存器1
//	MPU6050_WriteReg(MPU6050_PWR_MGMT_2, 0x00); // 電源管理寄存器2
//	
//	MPU6050_WriteReg(MPU6050_SMPLRT_DIV, 0x09); // 採樣率分頻(10分頻)
//	MPU6050_WriteReg(MPU6050_CONFIG, 0x06); // 配置寄存器
//	MPU6050_WriteReg(MPU6050_GYRO_CONFIG, 0x18); // 陀螺儀配置寄存器
//	MPU6050_WriteReg(MPU6050_ACCEL_CONFIG, 0x00); // 加速度計配置寄存器
//	
//	MPU6050_WriteReg(MPU6050_RA_USER_CTRL, 0x10); // 不让MPU6050 控制AUXI2C
//	MPU6050_WriteReg(MPU6050_RA_INT_PIN_CFG, 0x02); // 主控制器的I2C与	MPU6050的AUXI2C	直通。控制器可以直接访问HMC5883L
...
{ 
	...
	if(!mpu_init())
  {
	  if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL))
		  OLED_ShowString(1, 1, "MPU Setting...");
...
//void accum_offset(float *Yaw_offset)
//{
